...
const EditEvalProcess = (props) => {
  ...
  useEffect(() => {
    if (clickedValue1.name === "All Themes") {
      ...
      setselectedTheme(selectedTheme?.filter((item) => item !== "All Themes"));
    }
  }, [clickedValue1]);
  ...
  useEffect(() => {
    if (clickedValue2.name === "All Languages") {
      ...
      setselectedLang(selectedLang?.filter((item) => item !== "All Languages"));
    }
  }, [clickedValue2]);
  useEffect(() => {
    if (evalID && evalID.district) {
      if (
        evalID.district.split(",").length === fulldistrictNames.length - 1 &&
  ...
  const handleclick = async () => {
    ...
    const value = { district: "", language: "", theme: "" };
